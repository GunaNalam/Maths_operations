...
function subtract(a, b) {
    if (isSmaller(a, b)) return '-' + subtract(b, a);  // Handling negative result
    ...
    return res.replace(/^0+/, ''); // Remove leading zeros
}
...
function multiply1(a, b) {
    ...
    return k.replace(/^0+/, ''); // Remove leading zeros
}
...
function func(e) {
    let a = document.getElementById('number1').value.trim();
    let b = document.getElementById('number2').value.trim();
    if (a.length == 0 || b.length == 0) alert("Please Enter both the numbers");
    else {
        let c = document.getElementById('Result'), t, i = 0;
        if (e == 'div') t = Math.floor(+a / +b) + '';  // Convert to numbers for division
